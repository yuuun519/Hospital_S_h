...
def card_detected(cardnumber):
    ...
    cursor.execute("SELECT * FROM patients_l WHERE cardnumber = %s", (cardnumber,))
...
def display_patient_info(result):
    clear_fields()  
    _, registrationnumber, name, dateofbirth, age, gender, phonenumber = result

    Ernumber.insert(0, registrationnumber or "")
    Ename.insert(0, name or "")
    Ebirth.insert(0, dateofbirth or "")
...
def update_patient():
    ...
    try:
        cursor.execute(
            "UPDATE patients_l SET name=%s, dateofbirth=%s, age=%s, gender=%s, phonenumber=%s WHERE registrationnumber=%s",
            (
                name or None,
                dateofbirth or None,
                age or None ,
...
def add_patient_data():
    ...
    try:
        cursor.execute("SELECT * FROM patients_l WHERE registrationnumber = %s", (registrationnumber,))
        result = cursor.fetchone()
        if not result:
        ...
        if update_fields:
            ...
            sql = "UPDATE patients_l SET " + ", ".join(update_fields) + " WHERE registrationnumber=%s"
...
def search():
    ...
    if patient_number:
        connection = connect_to_database()
        cursor = connection.cursor()
        cursor.execute("SELECT * FROM patients_l WHERE registrationnumber = %s", (patient_number,))
        result = cursor.fetchone()
        if result:
            display_patient_info(result) 
        else:
